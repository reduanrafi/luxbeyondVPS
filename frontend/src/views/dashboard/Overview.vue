<template>
    <div>
        <!-- Profile Summary -->
        <div
            class="bg-surface border border-white/5 p-6 mb-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div class="flex items-center gap-4">
                <div
                    class="w-16 h-16 rounded-full bg-primary flex items-center justify-center text-white font-bold text-xl shadow-md">
                    {{ userInitials }}
                </div>
                <div>
            ...
            <div class="flex flex-wrap gap-3">
                <router-link to="/dashboard/orders"
                    class="inline-flex items-center gap-2 px-4 py-2 rounded-sm text-sm font-bold text-white bg-primary hover:bg-white transition-colors">
                    View My Orders
            ...
            <div class="relative bg-surface p-6 border border-white/5 group hover:border-primary/50 transition-colors">
                <div class="relative flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-primary text-xs mt-2 font-medium">{{ stats.ordersThisMonth > 0 ?
                            `+${stats.ordersThisMonth} this month` : 'No orders this month' }}</p>
                    </div>
            ...
            <div class="relative bg-surface p-6 border border-white/5 group hover:border-primary/50 transition-colors">
                <div class="relative flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-primary text-xs mt-2 font-medium">{{ stats.pendingRequests > 0 ?
                            `${stats.pendingRequests} pending` : 'No pending requests' }}</p>
                    </div>
                    <div class="p-4 bg-primary/10 rounded-full">
        ...
        <div class="bg-surface border border-white/5 p-6 hover:border-primary/30 transition-colors">
            <div class="flex items-center justify-between mb-6">
                ...
                <router-link to="/dashboard/orders"
                    class="text-xs font-semibold text-primary hover:text-white transition-colors">View All</router-link>
            </div>
            ...
            <div v-else class="space-y-3">
                <div v-for="activity in activities" :key="activity.id"
                    ...
                    <div
                        class="mt-0.5 p-2 rounded-full bg-primary/10 transition-all group-hover:bg-primary group-hover:text-white text-primary">
                        <component :is="getActivityIcon(activity.type)" class="w-4 h-4" />
                    </div>
                    <div class="flex-1">
                        <p class="text-sm font-medium text-slate-300 group-hover:text-primary transition-colors">{{
                            activity.title }}</p>
